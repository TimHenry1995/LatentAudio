...
def run_on_layer(layer_index: int,
        latent_representations_folder_path: str, 
...
if __name__ == "__main__": 
    ...
    parser = argparse.ArgumentParser(
        ...
        description='''This script creates a standard scaler to be used before principal component analysis (PCA), a PCA model and a standard scaler to be used after PCA. 
                        ...
                        The here created models are not used to perform the projection of the data. Instead, the models are only fitted and saved at `PCA_and_standard_scaler_folder`. 
    ...
    parser.add_argument("--latent_representations_folder", help="A list of strings that, when concatenated using the os-specific separator, result in a path to a folder in which the latent representations are stored.", type=str)
    parser.add_argument("--PCA_and_standard_scaler_folder", help="A list of strings that, when concatenated using the os-specific separator, result in a path to a folder in which the models shall be stored.", type=str)
    ...
    parser.add_argument("--sample_sizes", help="A list of integers (or None entries) indicating for each layer in the layer_indices list which sample size shall be used. Keep in mind that in order to train a PCA mode, one needs at least as many unique instances as dimensions. If a None is specified, then 1.1 * the latent dimensionality of the current layer is used.", type=str)
    ...
    print("\n\nThe script create_scalers_and_PCA_model_for_latent_yamnet parsed the following arguments:")
    ...
    latent_representations_folder_path = 'E:\\LatentAudio\data\latent\original\latent_representations_folder'
    PCA_and_standard_scaler_folder_path = 'E:\\LatentAudio\models\pca and standard scalers'
    layer_indices = [0, 9, 13]
    target_dimensionalities = [64, None, 64]
    random_seed = 42
    # Create PCA model and standard scalers
    for l, layer_index in enumerate(layer_indices):
        run_on_layer(layer_index=layer_index,
            latent_representations_folder_path = latent_representations_folder_path,
            PCA_and_standard_scaler_folder_path = PCA_and_standard_scaler_folder_path,
